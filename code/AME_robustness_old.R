...
if (vary_only_start){
  analysis_result_file <- here(
    paste0("output/robustness/estimations_", variable_of_interest, "_red.csv"))
  analysis_result_file_mpdata <- here(
    paste0("output/robustness/mpdata_", variable_of_interest, "_red.csv"))
  pdf_file_name <- here(paste0(
    "output/robustness/robustness_check_red.pdf"))
  txt_file <- here(paste0("output/opec/period_info_red.txt"))
} else {
  analysis_result_file <- here(
    paste0("output/robustness/estimations_", variable_of_interest, ".csv"))
  analysis_result_file_mpdata <- here(
    paste0("output/robustness/mpdata_", variable_of_interest, ".csv"))
  pdf_file_name <- here(paste0(
    "output/robustness/opec_shit.pdf"))
...
if (redo_computation){
    ...
    for (ma in max_range) {
      ...
      reg_res <- tryCatch(expr = {
        ...
        cplot_data_gdp <- cplot(
          ...
          what = "effect", 
          draw = FALSE,
          xvals = seq(-2.4, 2.4, 0.1)
        )
        
        cplot_data_eci_final <- tibble(cplot_data_eci) %>%
          mutate(dx_variable="eci") 
        cplot_data_gdp_final <- tibble(cplot_data_gdp) %>%
          mutate(dx_variable="GDP") 
        cplot_data_final <- rbind(
          cplot_data_eci_final, cplot_data_gdp_final) %>%
  ...
  fwrite(full_plot_list, file = here("output/robustness/robustness_est.csv"))
}

# Visualization----------------------------------

plot_data <- fread(here("output/robustness/robustness_est.csv"))

robustness_gdp <- plot_data %>%
  filter(first_year<2010, dx_variable=="eci") %>%
  ggplot(., aes(x=xvals, y=yvals, group=period)) +
  geom_line(color="grey", alpha=0.75) + 
  labs(
    title = "AME of initial GDP",
    x = "ECI", y = "AME of initial GDP per capita"
  ) +
  theme_icae()

robustness_eci <- plot_data %>%
  filter(first_year<2010, dx_variable=="GDP") %>%
  ggplot(., aes(x=xvals, y=yvals, group=period)) +
  geom_line(color="grey", alpha=0.75) + 
  labs(
    title = "AME of ECI",
    x = "Initial GDP per capita", y = "AME of ECI"
  ) +
  theme_icae()

robustness_full <- ggpubr::ggarrange(
  robustness_eci, robustness_gdp, ncol = 2, labels = c("A)", "B)")
)

robustness_full <- ggpubr::annotate_figure(
  robustness_full, 
  top = ggpubr::text_grob("Average marginal effects for varying starting years", size = 14))

ggsave(plot = robustness_full, 
       filename = here("output/fig_4_robustness.pdf"), 
       width = 9, height = 4)
